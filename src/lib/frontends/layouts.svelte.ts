...
export const fetchLayout = async (childLayout: string | null = null) => {
    ...
    if(get(settings).BACKEND === 'github') {
        ...
        return {
            baseLayout : layouts.find((obj: any) => obj['name'] === get(settings).BASE_LAYOUT).object.text,
            childLayout : childLayout ? layouts.find((obj: any) => obj['name'] === childLayout).object.text : null
        };
...
export const renderLayout = (layouts: any, child: string, allData: any) => {
    ...
    switch (extension[extension.length - 1]) {
        case 'md':
            ...
            break;
        case 'njk':
            html = renderNunjucks(layouts.baseLayout, { content : renderChildLayout(child, layouts.childLayout, allData) });
            break;
        case 'liquid':
            html = renderLiquid(layouts.characterLayout,  { content : renderChildLayout(child, layouts.childLayout, allData) });
            break;
        case 'hbs':
            html = renderHandlebars(layouts.characterLayout,  { content : renderChildLayout(child, layouts.childLayout, allData) });
            break;
        default:
            throw new Error("Could not render, did not find matching extension");
    }
...
export const renderChildLayout = (child: string, layout: any, allData: any) => {
    ...
    switch (extension[extension.length - 1]) {
        ...
        case 'hbs':
            ...
            break;
        default:
            throw new Error("Could not render preview, did not find matching extension");
    }
