...
import { fetchImagesGithub, putFileGithub } from "$lib/backends/github.svelte";
...
export const fetchImages = async () => {
    if (get(settings).BACKEND === 'github') {
        images.set((await fetchImagesGithub()).sort((a, b) => { 
...
export const writeImage = async (filename: string, formData: FormData) => {
    ...
    if (get(settings).BACKEND === 'github') {
        let result = await putFileGithub(get(settings).IMAGE_DIRECTORY + "/" + filename  + '.md', body);
        console.log(result);
    } else if (get(settings).BACKEND === 'forgejo') {
        //TODO
    } else {
        throw new Error("Could not put image markdown file, backend not defined");
    }
    ...
    try {
        ...
        let result = await putFileGithub(get(settings).MEDIA_PATH + "/" + filename, imageBody);
        console.log(result);
    } catch(ex) {
 ...
 */
const fileToBase64 = (blob: Blob | File) => new Promise((resolve, reject) => {
